    ...
    def load_weights(self, override_key: str | None = None, device: str = "cpu"):
        ...
        for key in keys:
            if self.gguf_loader.has_tensor(key + ".ffn_gate_exps.weight"):
                ...
                gate_type = self.gguf_loader.tensor_info[translated_key + ".ffn_gate_exps.weight"]["ggml_type"]
                up_type = self.gguf_loader.tensor_info[translated_key + ".ffn_up_exps.weight"]["ggml_type"]
                down_type = self.gguf_loader.tensor_info[translated_key + ".ffn_down_exps.weight"]["ggml_type"]
...
class KExpertsCPU(KExpertsBase):
    ...
    def load(self, w: dict | nn.Parameter | tuple | None = None, device:str|None = None, warmup:bool = False):
        ...
        if warmup and hasattr(self, 'moe'):
        ...
        if KExpertsCPU.input_tensor_cpu == None:
            # Check if we can use pin_memory
            use_pin_memory = torch.cuda.is_available() or torch.xpu.is_available()
            
            if isinstance(cuda_graphs, list):
                KExpertsCPU.input_tensor_cpu = [torch.zeros((cuda_graphs[i], self.config.hidden_size), device="cpu", pin_memory=use_pin_memory) for i in range(len(cuda_graphs))]
                KExpertsCPU.expert_ids_cpu = [torch.zeros((cuda_graphs[i], num_experts_per_tok), device="cpu", dtype=torch.long, pin_memory=use_pin_memory) for i in range(len(cuda_graphs))]
                KExpertsCPU.weights_cpu = [torch.zeros((cuda_graphs[i], num_experts_per_tok), device="cpu", dtype=torch.float32, pin_memory=use_pin_memory) for i in range(len(cuda_graphs))]
                KExpertsCPU.output_cpu = [torch.zeros((cuda_graphs[i], self.config.hidden_size), device="cpu", pin_memory=use_pin_memory, dtype=torch.bfloat16) for i in range(len(cuda_graphs))]
                KExpertsCPU.bsz_tensor_cpu = [torch.zeros((1), device="cpu", dtype=torch.int32, pin_memory=use_pin_memory) for i in range(len(cuda_graphs))]
            else:
                KExpertsCPU.input_tensor_cpu = torch.zeros((cuda_graphs, self.config.hidden_size), device="cpu", pin_memory=use_pin_memory)
                KExpertsCPU.expert_ids_cpu = torch.zeros((cuda_graphs, num_experts_per_tok), device="cpu", dtype=torch.long, pin_memory=use_pin_memory)
                KExpertsCPU.weights_cpu = torch.zeros((cuda_graphs, num_experts_per_tok), device="cpu", dtype=torch.float32, pin_memory=use_pin_memory)
                if torch.xpu.is_available():
                    KExpertsCPU.output_cpu = torch.zeros((cuda_graphs, self.config.hidden_size), device="cpu", pin_memory=use_pin_memory, dtype=model_dtype)
                    KExpertsCPU.bsz_tensor_cpu = torch.ones((1), device="cpu", dtype=torch.int32, pin_memory=use_pin_memory)
                else:
                    KExpertsCPU.output_cpu = torch.zeros((cuda_graphs, self.config.hidden_size), device="cpu", pin_memory=use_pin_memory, dtype=torch.bfloat16)
                    KExpertsCPU.bsz_tensor_cpu = torch.zeros((1), device="cpu", dtype=torch.int32, pin_memory=use_pin_memory)
    ...
    def forward(self, input_tensor, expert_ids, weights, bsz_tensor=None, cuda_graph_idx=0):
        ...
        if not hasattr(self, 'moe'):
            raise RuntimeError(f"KExpertsCPU.moe not initialized. Call load() before forward(). Current backend: {getattr(self, 'backend', 'unknown')}")
    ...
    def load_weights(self, override_key: str | None = None, device: str = "cpu"):
        ...
        for key in keys:
            if isinstance(self.gguf_loader, SafeTensorLoader):
                ...
                # For GGUF, translate the key to match GGUF naming
                from ktransformers.util.custom_gguf import translate_name_to_gguf
                # Remove .experts suffix and translate the base path
                if key.endswith('.mlp.experts'):
                    base_key = key[:-len('.mlp.experts')]
                    translated_key = translate_name_to_gguf(base_key)
                else:
                    translated_key = translate_name_to_gguf(key)
                    
            if self.gguf_loader.has_tensor(translated_key + ".ffn_gate_exps.weight"):
                gate = self.gguf_loader.get_mmap_tensor(translated_key + ".ffn_gate_exps.weight")
                up = self.gguf_loader.get_mmap_tensor(translated_key + ".ffn_up_exps.weight")
                down = self.gguf_loader.get_mmap_tensor(translated_key + ".ffn_down_exps.weight")
                # gate_type = self.gguf_loader.tensor_info[key + ".ffn_gate_exps.weight"]["ggml_type"]
                # up_type = self.gguf_loader.tensor_info[key + ".ffn_up_exps.weight"]["ggml_type"]
                # down_type = self.gguf_loader.tensor_info[key + ".ffn_down_exps.weight"]["ggml_type"]
                gate_type = self.gguf_loader.get_ggml_type(translated_key + ".ffn_gate_exps.weight")
                up_type = self.gguf_loader.get_ggml_type(translated_key + ".ffn_up_exps.weight")
                down_type = self.gguf_loader.get_ggml_type(translated_key + ".ffn_down_exps.weight")
            
            elif translated_key + ".ffn_gate_exps.weight" in self.gguf_loader.tensor_info:
                gate = self.gguf_loader.get_mmap_tensor(translated_key + ".ffn_gate_exps.weight")
                up = self.gguf_loader.get_mmap_tensor(translated_key + ".ffn_up_exps.weight")
                down = self.gguf_loader.get_mmap_tensor(translated_key + ".ffn_down_exps.weight")
                gate_type = self.gguf_loader.tensor_info[translated_key + ".ffn_gate_exps.weight"]["ggml_type"]
                up_type = self.gguf_loader.tensor_info[translated_key + ".ffn_up_exps.weight"]["ggml_type"]
                down_type = self.gguf_loader.tensor_info[translated_key + ".ffn_down_exps.weight"]["ggml_type"]
            elif translated_key + ".ffn_down.0.weight" in self.gguf_loader.tensor_info:
                # for supporting  Mixtral-8x7B-Instuct  
                gate = []
                up = []
                down = []
                for i in range(8):
                    gate_it = self.gguf_loader.get_mmap_tensor(f"{translated_key}.ffn_gate.{i}.weight")
                    up_it = self.gguf_loader.get_mmap_tensor(f"{translated_key}.ffn_up.{i}.weight")
                    down_it = self.gguf_loader.get_mmap_tensor(f"{translated_key}.ffn_down.{i}.weight")
                    gate.append(gate_it)
                    up.append(up_it)
                    down.append(down_it)
                gate = np.stack(gate)
                up = np.stack(up)
                down = np.stack(down)
                gate_type = self.gguf_loader.get_ggml_type(translated_key + ".ffn_gate.0.weight")
                up_type = self.gguf_loader.get_ggml_type(translated_key + ".ffn_up.0.weight")
                down_type = self.gguf_loader.get_ggml_type(translated_key + ".ffn_down.0.weight")
            else:
                raise ValueError(f"Experts {key} not found in gguf_loader")
            res = {key:{"gate": gate, "up": up, "down": down, "gate_type": gate_type, "up_type": up_type, "down_type": down_type}}
        return res
