...
if __name__ == "__main__":
    ...
    kv_len = 4023
    q_len = 1
    q_nope_buf = torch.randn((max_batch_tokens, num_heads, 512), dtype=torch.bfloat16, device="cpu")
    q_pe_buf = torch.randn((max_batch_tokens, num_heads, 64), dtype=torch.bfloat16, device="cpu")
    kv_buf = torch.randn((max_pages, page_size, 576), dtype=torch.bfloat16, device="cpu")
    ...
    kv_len_arr = torch.tensor([kv_len], dtype=torch.int32, device="cpu")
    qo_indptr = torch.tensor([0, q_len], dtype=torch.int32, device="cpu")
    kv_indptr = torch.tensor([0, used_pages], dtype=torch.int32, device="cpu")
    kv_indices = torch.empty(max_pages, dtype=torch.int32, device="cpu")
    kv_indices[:used_pages] = torch.arange(0, used_pages, dtype=torch.int32, device="cpu")
    bsz_tensor = torch.tensor([1], dtype=torch.int32, device="cpu")
    wrapper.plan(
        qo_indptr,
        kv_indptr,
        kv_indices,
    ...
    pages = max_pages
    used_pages = (kv_len + page_size - 1)// page_size
    q_nope = torch.randn((q_len*2, num_heads, 512), dtype=torch.bfloat16, device="cpu")
    q_nope[q_len:] = q_nope[:q_len]
    q_pe = torch.randn((q_len*2, num_heads, 64), dtype=torch.bfloat16, device="cpu")
    q_pe[q_len:] = q_pe[:q_len]
    kv_cache = torch.randn((max_pages, page_size, 576), dtype=torch.bfloat16, device="cpu")
    kv_cache[used_pages:2*used_pages] = kv_cache[:used_pages]
    ckv, k_pe = torch.split(kv_cache, [512, 64], dim=-1)
    
    kv_len_arr = torch.tensor([kv_len, kv_len], dtype=torch.int32, device="cpu")
    qo_indptr = torch.tensor([0, q_len, q_len*2], dtype=torch.int32, device="cpu")
    kv_indptr = torch.tensor([0, used_pages, used_pages*2], dtype=torch.int32, device="cpu")
    kv_indices = torch.empty(max_pages, dtype=torch.int32, device="cpu")
    kv_indices[:2*used_pages] = torch.arange(0, 2*used_pages, dtype=torch.int32, device="cpu")
    bsz_tensor = torch.tensor([2], dtype=torch.int32, device="cpu")
    wrapper.plan(
        qo_indptr,
        kv_indptr,
        kv_indices,
    ...
    for forward_id in range(0, 1):
        ...
        for layer_id in range(1):
            ...
            q_nope = torch.load(os.path.join(flashinfer_folder, file_name)).view(q_len,128,512).to(device="cpu")
            file_name = f"layer_{layer_id}_forward_{forward_id}_q_pe.pt"
            q_pe = torch.load(os.path.join(flashinfer_folder, file_name)).view(q_len,128,64).to(device="cpu")
            q = torch.cat([q_nope, q_pe], dim=-1)
            kv_cache = torch.load(kv_cache_path).to(device="cpu")
            ...
            page_table = torch.arange(max_pages, dtype=torch.int32, device="cpu")
