...
def load_cur_state_dict(module: nn.Module, gguf_loader: GGUFLoader, prefix: str = ""):
    ...
    for name, param in local_state.items():
        ...
        if translated_key in gguf_loader.tensor_file_map:
            ...
            # device = "cpu" if "embd" in translated_key else "cpu"
...
def warmup(module:nn.Module):
    if not isinstance(module, base_operator.BaseInjectedModule):
        for _, child in module._modules.items():
            warmup(child)
    else:
        if hasattr(module, "warmup") and callable(module.warmup):
            module.warmup()
...
def prefill_and_generate(model, tokenizer, inputs, max_new_tokens=10000, use_cuda_graph: bool = True,
    ...
    def decode_one_tokens(decode_cuda_graph_runner, cur_token, position_ids, cache_position, past_key_values, use_cuda_graph: bool = True):
        if use_cuda_graph:
    ...
    with torch.no_grad():
        ...
        if mode == "long_context":
            ...
            if use_cuda_graph:
                # warm up
                prefill(None, inputs.to("cpu"), None, cache_position, past_key_values, use_cuda_graph=False).to(torch_device)
                # capture
                prefill_cuda_graph_runner = CUDAGraphRunner()
                prefill_cuda_graph_runner.capture(model, inputs.to("cpu"), None, cache_position, past_key_values, torch_device, return_dict=False, use_cache=True)
            else:
                prefill_cuda_graph_runner = None
            start_time = time.time()
            next_token = prefill(prefill_cuda_graph_runner, inputs.to("cpu"), None, cache_position, past_key_values, use_cuda_graph).to(torch_device)
        ...
        if use_cuda_graph:
            decode_cuda_graph_runner = CUDAGraphRunner()
            decode_cuda_graph_runner.capture(model, next_token.unsqueeze(0), position_ids, cache_position, past_key_values, torch_device, return_dict=False, use_cache=True)
        else:
            decode_cuda_graph_runner = None
            
        start_time = time.time()
        for _ in range(1, max_new_tokens):
            next_token = decode_one_tokens(decode_cuda_graph_runner, next_token.unsqueeze(0), position_ids, cache_position, past_key_values, use_cuda_graph).to(torch_device)
            inputs = torch.cat((inputs, next_token.unsqueeze(0)), dim=-1)
